...
read_env <- function(env_file){
	env <- read.table(env_file, header = TRUE)
	if ("FID" %in% names(cov) & "IID" %in% names(cov)){
        return(cov)
        } else {
        print("FID and IID not present in the covariate file")
...
association.fun <- function(gene){
  pred_gene_exp <- merged[[gene]]
  if (is_cov) {
  fml <- as.formula(paste("phenotype ~ pred_gene_exp * ",paste0(env_name,"+") , paste(cov.names,collapse = "+")))	
  } else {
  fml <- as.formula(paste("phenotype ~ pred_gene_exp * ",env_name))
  }
  if (test_type == "logistic"){
  ...
  return (c(gene = gene,unlist(res[paste0("pred_gene_exp:",env_name),])))
}
...
#read environment file and merge

env_file <- read_env(argv$ENV_FILE)
merged <- merged(env_file,merged,by = c(1,2), sort = FALSE)
env_name <- names(env_file)[3]
